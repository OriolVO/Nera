...
impl<'a> SemanticAnalyzer<'a> {
    ...
    fn substitute_type_in_expr(&mut self, expr: &mut Expression, subs: &HashMap<String, Type>) {
        match expr {
            ...
            Expression::Cast(c) => self.substitute_type_in_expr(&mut c.expr.node, subs),
    ...
    fn analyze_statement(&mut self, stmt: &mut Spanned<Statement>) {
        match &mut stmt.node {
            ...
            Statement::Break => {
                if self.in_loop == 0 {
                    self.report_error(&stmt.span, "Cannot 'break' outside of a loop.");
                }
            }
            Statement::Continue => {
                if self.in_loop == 0 {
                    self.report_error(&stmt.span, "Cannot 'continue' outside of a loop.");
                }
            }
    ...
    fn analyze_expression(&mut self, expr: &mut Spanned<Expression>) -> TypeInfo {
        ...
        match &mut expr.node {
            ...
            Expression::Binary(bin_expr) => {
                ...
                match bin_expr.op {
                    ...
                    BinaryOp::Mod | BinaryOp::BitAnd | BinaryOp::BitOr | BinaryOp::BitXor | BinaryOp::Shl | BinaryOp::Shr => {
                        ...
                        let left_inner = if left_ty.is_array() { left_ty.get_array_inner().unwrap() } else { left_ty.clone() };
                        let right_inner = if right_ty.is_array() { right_ty.get_array_inner().unwrap() } else { right_ty.clone() };

                        if left_inner != TypeInfo::Int && left_inner != TypeInfo::Unknown {
                            self.report_error(&bin_expr.left.span, "Modulo and Bitwise operations require Int operands");
                        }
                        if right_inner != TypeInfo::Int && right_inner != TypeInfo::Unknown {
                            self.report_error(&bin_expr.right.span, "Modulo and Bitwise operations require Int operands");
                        }
            ...
            Expression::Cast(cast_expr) => {
                let source_ty = self.analyze_expression(&mut cast_expr.expr);
                let target_ty = self.resolve_type_ast(&cast_expr.target_type.node);
